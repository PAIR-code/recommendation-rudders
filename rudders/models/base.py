...
class CFModel(tf.keras.Model, abc.ABC):
    ...
    def score(self, lhs, lhs_biases, rhs, rhs_biases, eval_mode):  # note that user biases are never used
    ...
    def random_eval(self,
        ...
        for counter, input_tensor in enumerate(examples.batch(batch_size)):
            if batch_size * counter >= total_examples:
                break
            scores, targets = self.get_scores_targets(
                input_tensor)  # score: score to all; targets: score to valid/eval item
            ...
            ranks[counter * batch_size:(counter + 1) * batch_size] += np.sum(
                # instead of doing this which is very error-prone, I can append to a list and concatenate at the end
                (scores >= targets), axis=1)
